...
export function BasicInfoStep({ formData, jurisdictions, industries, onUpdate, errors }: BasicInfoStepProps) {
  ...
  React.useEffect(() => {
    if (calculatedDuration && calculatedDuration !== "Invalid date range" && calculatedDuration !== "") {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div>
          <Label htmlFor="documentName">Document Name <span className="text-destructive">*</span></Label>
          <Input
            id="documentName"
          ...
          />
          {errors?.documentName && (
            <p className="text-sm text-destructive mt-1">{errors.documentName}</p>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            {errors?.jurisdiction && (
              <p className="text-sm text-destructive mt-1">{errors.jurisdiction}</p>
            )}
          </div>
          ...
          <div>
            ...
            >
              <SelectTrigger className={`h-10 rounded-sm mt-1.5 ${errors?.industry ? 'border-destructive' : ''}`}>
